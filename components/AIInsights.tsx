...
import { Sparkles, BrainCircuit, RefreshCcw, ShieldAlert } from 'lucide-react';
    ...
    <div className="space-y-6">
      ...
      <div className="bg-[#111418] rounded-[2rem] p-10 border border-[#1c2127] shadow-2xl relative overflow-hidden">
        <div className="absolute top-0 right-0 p-8 opacity-[0.03] pointer-events-none">
          <Sparkles size={240} className="text-[#2ebd85]" />
        </div>
        
        {loading ? (
          <div className="space-y-6">
            <div className="h-6 bg-slate-800/30 animate-pulse rounded-full w-3/4"></div>
            <div className="h-4 bg-slate-800/20 animate-pulse rounded-full w-1/2"></div>
            <div className="h-4 bg-slate-800/20 animate-pulse rounded-full w-5/6"></div>
            <div className="h-32 bg-slate-800/10 animate-pulse rounded-3xl w-full mt-8"></div>
          </div>
        ) : (
          <div className="prose prose-invert max-w-none prose-emerald prose-headings:text-[#2ebd85] prose-headings:font-black prose-headings:uppercase prose-headings:tracking-tighter prose-p:text-slate-300 prose-p:leading-relaxed prose-strong:text-white prose-li:text-slate-300">
            {analysis ? analysis.split('\n').map((line, i) => {
              if (line.startsWith('#')) return <h3 key={i} className="text-xl mt-8 mb-4">{line.replace(/#/g, '').trim()}</h3>;
              if (line.startsWith('-') || line.startsWith('*')) return <li key={i} className="ml-4 mb-2">{line.substring(1).trim()}</li>;
              if (line.trim() === '') return <div key={i} className="h-4" />;
              return <p key={i} className="mb-4">{line}</p>;
            }) : (
              <div className="text-center py-20 opacity-30 font-black uppercase tracking-widest">Initial Analysis Pending...</div>
            )}
          </div>
        )}
      </div>
      ...
      <div className="bg-amber-500/5 border border-amber-500/20 p-5 rounded-2xl flex gap-4 items-start">
        ...
        <p className="text-[11px] text-amber-500/70 leading-normal font-medium">
          <strong className="text-amber-500 uppercase font-black block mb-1">Risk Disclosure</strong>
          AI-generated insights are for educational purposes within the paper trading environment only. Real market conditions are volatile and complex. Nexora AI does not provide financial advice.
        </p>
      </div>
    </div>
  );

    ...
    const simInterval = setInterval(() => {
      ...
      if (now - lastRtdbSyncRef.current >= RTDB_SYNC_INTERVAL) {
        ...
        updatedStocks.forEach(s => {
          updatePayload[s.Symbol] = { ltp: s.LTP, updatedAt: now, source: 'Admin_Live_Drift' };
        });
        rtdbUpdate(ref(rtdb, 'market/snapshot/stocks'), updatePayload);
    ...
    <div className="flex flex-col h-screen bg-[#080a0c] text-slate-200 overflow-hidden font-sans">
        ...
        <div className="flex items-center gap-10">
          ...
          <nav className="flex items-center gap-1 bg-[#080a0c] p-1 rounded-lg border border-[#1c2127]">
            {[{ id: AppTab.MARKET, icon: LayoutDashboard, label: 'Terminal' }, { id: AppTab.PORTFOLIO, icon: Briefcase, label: 'Portfolio' }, { id: AppTab.AI_INSIGHTS, icon: Cpu, label: 'Analysis' }].map(t => (
              <button key={t.id} onClick={() => setActiveTab(t.id)} className={`flex items-center gap-2 px-4 py-1.5 rounded-md text-[11px] font-black uppercase transition-all ${activeTab === t.id ? 'bg-[#1c2127] text-[#2ebd85]' : 'text-slate-500 hover:text-slate-300'}`}><t.icon size={14} />{t.label}</button>
            ))}
          </nav>
        </div>

        <div className="flex items-center gap-6">
          <div className={`flex items-center gap-3 bg-[#080a0c] px-3 py-1 rounded-full border border-[#1c2127] ${isMarketOpen ? 'border-[#2ebd85]/30' : 'border-rose-500/30'}`}>
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        {activeTab === AppTab.AI_INSIGHTS && (
          <div className="flex-1 overflow-y-auto custom-scrollbar p-8 bg-[#080a0c]">
            <div className="max-w-4xl mx-auto"><AIInsights stocks={stocks} /></div>
          </div>
        )}
      </div>
    </div>
